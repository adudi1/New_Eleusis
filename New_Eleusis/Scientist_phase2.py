    ...
    def forward_checking(self, domain):
        ...
        for rule in domain:
            ...
            if legalValue:
                legalValue2 = True
                for i in range(0,len(self.prevCards)-2,1):
                    k = len(self.prevCards[i+2][1])
                    if k >0:
                        for j in range(0,k,1):
                            cards = (self.prevCards[i][0],self.prevCards[i+1][0],self.prevCards[i+2][0])
                            try:
                                legalValue2 = p.evaluate(cards)
...
#
# setRule("equal(color(current), R)")
# scientist(['3H', '2D', 'JH'],[generate_random_card() for i in range(14)], False)
# scientist(['5D', '3H', 'QS'],None, False)
# scientist(['3S', '5S', 'AH'],None, False)
